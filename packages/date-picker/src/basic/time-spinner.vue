<template>
  <div class="el-time-spinner" :class="{ 'has-seconds': showSeconds, 'only-hours': !showMinutes }">
    <template v-if="!arrowControl">
      ...
      <el-scrollbar
        ...
        ref="seconds">
        <li
          @click="handleClick('seconds', { value: key, disabled: is24h && key !== 0 })"
          v-for="(second, key) in 60"
          class="el-time-spinner__item"
          :class="{ 'active': key === seconds, 'disabled': is24h && key !== 0 }"
...
<script type="text/babel">
  ...
  export default {
    ...
    computed: {
      hours() {
        return this.is24h ? 24 : this.date.getHours();
      },
      minutes() {
      ...
      hoursList() {
        const base = getRangeHours(this.selectableRange);
        return this.allow24h ? base.concat([false]) : base;
      },
      minutesList() {
        if (this.is24h) {
          const list = new Array(60).fill(false);
          list[0] = true;
          return list;
        }
      ...
      arrowHourList() {
        ...
        const max = this.allow24h ? 24 : 23;
        return [
          hours > 0 ? hours - 1 : undefined,
          hours,
          hours < max ? hours + 1 : undefined
        ];
      },
      arrowMinuteList() {
    ...
    methods: {
      ...
      modifyDateField(type, value) {
        switch (type) {
          case 'hours':
            if (this.allow24h && value === 24) {
              const d = modifyTime(this.date, 0, 0, 0);
              d._24h = true;
              this.$emit('change', d);
            } else {
            ...
            break;
          case 'minutes':
            if (this.is24h && this.allow24h) {
              if (value !== 0) break;
              const dm = modifyTime(this.date, 0, 0, 0);
              dm._24h = true;
              this.$emit('change', dm);
            } else {
          ...
          case 'seconds':
            if (this.is24h && this.allow24h) {
              if (value !== 0) break;
              const ds = modifyTime(this.date, 0, 0, 0);
              ds._24h = true;
              this.$emit('change', ds);
            } else {
      ...
      emitSelectRange(type) {
        if (type === 'hours') {
          ...
          const start = this.showMinutes ? 6 : 3;
          this.$emit('select-range', start, start + 2);
        }
      ...
      bindScrollEvent() {
        ...
        bindFunction('hours');
        if (this.showMinutes) bindFunction('minutes');
        if (this.showSeconds) bindFunction('seconds');
      },

      handleScroll(type) {
        const value = Math.min(Math.round((this.$refs[type].wrap.scrollTop - (this.scrollBarHeight(type) * 0.5 - 10) / this.typeItemHeight(type) + 3) / this.typeItemHeight(type)), (type === 'hours' ? (this.allow24h ? 24 : 23) : 59));
        this.modifyDateField(type, value);
      },
      ...
      adjustSpinners() {
        ...
        if (this.showMinutes) this.adjustSpinner('minutes', this.minutes);
        if (this.showSeconds) this.adjustSpinner('seconds', this.seconds);
      },
      ...
      scrollDown(step) {
        ...
        if (this.currentScrollbar === 'hours') {
          ...
          if (this.is24h && this.allow24h) return;
          now = (now + step + 60) % 60;
        }
