  ...
  methods: {
    ...
    getCellClass(rowIndex, columnIndex, row, column) {
      ...
      if (column.borderConfig) {
        ...
        const tableBorder = this.table.border || this.table.isGroup;
    ...
    handleCellMouseEnter(event, row) {
      ...
      // 支持竖向 hover 斑马纹
      try {
        const stripeColumnProps = table.stripeColumnProps || { hover: false };
        if (stripeColumnProps.hover && cell) {
          if (table.highlightCurrentColumn) {
            table.suspendCurrentColumnHighlight = true;
          }
          // 提取当前单元格列 id
          const matches = (cell.className || '').match(/el-table_[^\s]+/gm);
          if (matches && matches[0]) {
            const colId = matches[0];
            // 直接查找所有属于该列 id 的 td，避免 colspan 时索引错位
            const selector = 'td.el-table__cell.' + colId;
            const sameColumnCells = this.$el.querySelectorAll(selector);
            sameColumnCells.forEach(td => td.classList.add('hover-column'));
            // 兼容：给当前列所有 td 添加 hover-column
            const tdList = this.$el.querySelectorAll('td.' + colId);
            if (tdList && tdList.length) {
              tdList.forEach(td => td.classList.add('hover-column'));
            }
          }
        }
      } catch (e) { /* ignore */ }
    ...
    handleCellMouseLeave(event) {
      ...
      // 移除 hover-column
      try {
        const stripeColumnProps = this.table.stripeColumnProps || { hover: false };
        if (stripeColumnProps.hover) {
          const cells = this.$el.querySelectorAll('td.hover-column');
          cells.forEach(td => td.classList.remove('hover-column'));
          const tds = this.$el.querySelectorAll('td.hover-column');
          tds.forEach(td => td.classList.remove('hover-column'));
        }
      } catch (e) { /* ignore */ }
