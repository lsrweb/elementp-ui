...
When `allow24h` is enabled, `24` is added at the end of the hour list to represent the end of the day (24:00:00). Minutes and seconds are automatically locked to `00` when hour 24 is selected.

:::demo When used with `value-format="HH:mm:ss"`, the binding value is the string `"24:00:00"`. Without `value-format`, the binding value is a Date object (midnight of the next day).
```html
<template>
  <el-time-picker
    v-model="value1"
  ...
  <el-time-picker
    ...
    allow24h
    value-format="HH:mm:ss"
    placeholder="Arrow mode with 24:00">
  </el-time-picker>
</template>
...
Use the `format` attribute to control which columns the time panel shows. `HH:mm` shows only hours and minutes, `HH` shows hours only.

:::demo `format` controls the panel columns displayed, while `value-format` controls the binding value format. Works with `allow24h` too.
```html
<template>
  <el-time-picker
    v-model="value1"
...
| allow24h | whether to allow selecting 24:00, only works with `<el-time-picker>` | boolean | — | false |
